...
public interface CredentialsMapper {
    ...
    public List<Credentials> findCredentialsByUserId(Integer userId);
    ...
    public Integer deleteCredentialsByIdAndUserId(Integer credentialId, Integer userId);

    // Update credentials for a credentialId and userId
    @Update("Update Credentials set url=#{url}, username=#{username}, key=#{key}, password=#{password}" +
            " where credentialId=#{credentialId} and userId=#{userId}")
    public Credentials updateCredentialsByCredentialIdAndUserId(Credentials credentials);
